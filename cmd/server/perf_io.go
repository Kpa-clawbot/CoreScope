...
func (s *Server) handlePerfSqlite(w http.ResponseWriter, r *http.Request) {
	...
	if s.db != nil && s.db.conn != nil && s.db.IsSQLite() {
...
func (s *Server) handlePerfDB(w http.ResponseWriter, r *http.Request) {
	if s.db == nil || s.db.conn == nil {
		writeJSON(w, DBPerfStats{Engine: ""})
		return
	}
	writeJSON(w, s.db.GetDBPerfStatsTyped())
}
