...
//
// Includes inactive_nodes as well as nodes: the infrastructure flag is
// operator-curated and independent of recent activity — a real anchor
// node that's gone quiet (retention-moved to inactive_nodes) is still
// infrastructure, and the admin panel's management UI needs to keep
// showing it so an operator can find it again to un-flag it. Without
// this, toggling a since-gone-stale node on would make it vanish from
// its own management list.
func (db *DB) GetInfrastructureNodes() ([]map[string]interface{}, error) {
	if !db.hasInfrastructure {
		return []map[string]interface{}{}, nil
	}
	cols := db.nodeSelectCols()
	rows, err := db.conn.Query(fmt.Sprintf(
		`SELECT %s FROM nodes WHERE infrastructure = 1
		 UNION ALL
		 SELECT %s FROM inactive_nodes WHERE infrastructure = 1
		 ORDER BY last_seen DESC`, cols, cols))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
