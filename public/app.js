...
function navigate() {
  ...
  if (basePage === 'tools') {
    if (routeParam && routeParam.startsWith('trace/')) {
      ...
      routeParam = null;
    } else if (routeParam === 'new-nodes') {
      basePage = 'new-nodes-tool';
      routeParam = null;
    } else if (routeParam === 'node-changes') {
      basePage = 'node-changes-tool';
      routeParam = null;
    } else if (!routeParam) {
  ...
  document.querySelectorAll('.nav-link[data-route]').forEach(el => {
    el.classList.toggle('active', el.dataset.route === basePage || (el.dataset.route === 'tools' && (basePage === 'traces' || basePage === 'path-inspector' || basePage === 'observer-neighbors-tool' || basePage === 'new-nodes-tool' || basePage === 'node-changes-tool' || basePage === 'tools-landing')));
  });
