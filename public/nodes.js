    ...
    try {
      ...
      body.innerHTML = `
        ...
        <table class="node-stats-table" id="node-stats">
          ...
          ${(n.role === 'repeater' || n.role === 'room') && Array.isArray(n.transported_scopes) && n.transported_scopes.length ? `<tr id="row-transported-scopes"><td title="Distinct region scopes (transmissions.scope_name) of all non-advert packets in which this repeater appears as a path hop. Shows which regions' traffic this repeater has carried (#1751).">Transported scopes</td><td>${n.transported_scopes.map(sc => '<span class="badge" style="margin:0 2px">' + escapeHtml(String(sc)) + '</span>').join(' ')}</td></tr>` : ''}
